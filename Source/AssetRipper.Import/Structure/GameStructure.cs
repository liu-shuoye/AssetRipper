...
public sealed class GameStructure : IDisposable
{
	...
	private GameStructure(List<string> paths, FileSystem fileSystem, CoreConfiguration configuration)
	{
		...
		InitializeGameCollection(
			...
			configuration.ImportSettings.GameType,
	...
	private void InitializeGameCollection(UnityVersion defaultVersion, UnityVersion targetVersion, GameType gameType, bool stripTexture2DData, bool stripMeshData, bool stripAudioClipData, DependencyMap? dependencyMap)
	{
		Logger.SendStatusChange("loading_step_create_file_collection");

		GameAssetFactory assetFactory = new(AssemblyManager, gameType, stripTexture2DData, stripMeshData, stripAudioClipData);
