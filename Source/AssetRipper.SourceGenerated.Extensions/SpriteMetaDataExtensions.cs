...
{
	/// <summary>
	/// 按资产的稳定标识计算确定性的 SpriteID（32 位小写十六进制）。
	/// </summary>
	/// <param name="sprite">目标精灵。</param>
	/// <returns>确定性 SpriteID 字符串。</returns>
	/// <remarks>
	/// SpriteID 在 Unity 与旧版导出中都是随机生成的；开启"确定性 GUID"导出后改用本方法，
	/// 使同一精灵在分批导出时 SpriteID 保持不变。键使用与导出 GUID 相同的稳定标识
	/// （类型、导出目录、名称、源文件、PathID），全部来自游戏数据，跨批次可复现。
	/// </remarks>
	public static string CalculateSpriteId(ISprite sprite)
	{
		string key = $"{sprite.ClassName}|{sprite.GetBestDirectory()}|{sprite.GetBestName()}|{sprite.Collection.Name}|{sprite.PathID}";
		return UnityGuid.Md5Hash(key).ToString();
	}
	...
	public static void FillSpriteMetaData(this ISpriteMetaData instance, ISprite sprite, ISpriteAtlas? atlas, bool useDeterministicSpriteId = false)
	{
		...
		if (instance.Has_Bones() && sprite.Has_Bones() && instance.Has_SpriteID())
		{
			...
			instance.SpriteID = useDeterministicSpriteId
				? CalculateSpriteId(sprite)
